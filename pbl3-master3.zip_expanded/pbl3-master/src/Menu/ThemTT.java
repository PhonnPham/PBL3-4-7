...
public class ThemTT extends JDialog {

	private static final long serialVersionUID = 1L;
	private final JPanel contentPanel = new JPanel();
	private jT txtHoten,txtAddress,txtSDT;
	private ThemTT qlthem = this;
	private int id;
	private home home;
	...
	public void initComponents() {
		...
		panel_2.setBounds(123, 61, 634, 357);
		...
		btnAdd.addActionListener(new ActionListener() {
			public void actionPerformed(ActionEvent e) {
				...
				if(!name.isEmpty() && date != null && !address.isEmpty() && !phone.isEmpty()
					&& !CCCD.isEmpty() && !username.isEmpty() && !password.isEmpty() && !email.isEmpty())
				{
//					if(DocgiaDAO.getInstance().checkDGSDT(phone))
//						 JOptionPane.showMessageDialog(contentPanel, "Số điện thoại đã tồn tại");
//					else
//					{	
						Thuthu tt = new Thuthu(++id,name, date, address, CCCD,phone, email, username,password);
						ttDao.getInstance().Insert(tt);
